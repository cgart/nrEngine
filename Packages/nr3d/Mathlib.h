...
#ifndef __NR_MATHLIB_H__
#define __NR_MATHLIB_H__
...
namespace nrEngine {
	...
	namespace nr3D {
		...
		/**
		* Structure storing two dimensional(x,y) vector with float32 values
		* \ingroup math
		*/
		struct vec2 {
			
			inline vec2() : x(0), y(0) { }
			inline vec2(float32 x,float32 y) : x(x), y(y) { }
			inline vec2(const float32 *v) : x(v[0]), y(v[1]) { }
			inline vec2(const vec2 &v) : x(v.x), y(v.y) { }
			
			inline int32 operator==(const vec2 &v) { return (fabs(x - v.x) < EPSILON && fabs(y - v.y) < EPSILON); }
			inline int32 operator!=(const vec2 &v) { return !(*this == v); }
			
			inline const vec2 operator*(float32 f) const { return vec2(x * f,y * f); }
			inline const vec2 operator/(float32 f) const { return vec2(x / f,y / f); }
			inline const vec2 operator+(const vec2 &v) const { return vec2(x + v.x,y + v.y); }
			inline const vec2 operator-() const { return vec2(-x,-y); }
			inline const vec2 operator-(const vec2 &v) const { return vec2(x - v.x,y - v.y); }
			
			inline vec2 &operator*=(float32 f) { return *this = *this * f; }
			inline vec2 &operator/=(float32 f) { return *this = *this / f; }
			inline vec2 &operator+=(const vec2 &v) { return *this = *this + v; }
			inline vec2 &operator-=(const vec2 &v) { return *this = *this - v; }
			
			inline float32 operator*(const vec2 &v) const { return x * v.x + y * v.y; }
			
			inline operator float32*() { return (float32*)&x; }
			inline operator const float32*() const { return (float32*)&x; }
			
			inline float32 &operator[](int32 i) { return ((float32*)&x)[i]; }
			inline const float32 operator[](int32 i) const { return ((float32*)&x)[i]; }
			
			inline float32 length() const { return sqrt(x * x + y * y); }
			inline float32 normalize() {
				float32 inv,length = sqrt(x * x + y * y);
		...
		/**
		* Structure storing three dimensional(x,y,z) vector with float32 values
		* \ingroup math
		*/
		struct vec3 {
			
			inline vec3() : x(0), y(0), z(0) { }
			inline vec3(float32 x,float32 y,float32 z) : x(x), y(y), z(z) { }
			inline vec3(const float32 *v) : x(v[0]), y(v[1]), z(v[2]) { }
			inline vec3(const vec3 &v) : x(v.x), y(v.y), z(v.z) { }
			inline vec3(const vec4 &v);
			
			inline int32 operator==(const vec3 &v) { return (fabs(x - v.x) < EPSILON && fabs(y - v.y) < EPSILON && fabs(z - v.z) < EPSILON); }
			inline int32 operator!=(const vec3 &v) { return !(*this == v); }
			
			inline const vec3 operator*(float32 f) const { return vec3(x * f,y * f,z * f); }
			inline const vec3 operator/(float32 f) const { return vec3(x / f,y / f,z / f); }
			...
			inline const vec3 operator*(const mat3 &mat) const;
			inline const vec3 operator*(const mat4 &mat) const;
			
			inline vec3 &operator*=(float32 f) { return *this = *this * f; }
			inline vec3 &operator/=(float32 f) { return *this = *this / f; }
			inline vec3 &operator+=(const vec3 &v) { return *this = *this + v; }
			inline vec3 &operator-=(const vec3 &v) { return *this = *this - v; }
			
			inline float32 operator*(const vec3 &v) const { return x * v.x + y * v.y + z * v.z; }
			inline float32 operator*(const vec4 &v) const;
			...
			inline float32 length() const { return sqrt(x * x + y * y + z * z); }
			inline float32 magnitude() const {return sqrt(x*x + y*y + z*z);}
		...
		/**
		* Structure storing four dimensional (x,y,z,w) vector with float32 values
		* \ingroup math
		*/
		struct vec4 {
			
			inline vec4() : x(0), y(0), z(0), w(1) { }
			inline vec4(float32 x,float32 y,float32 z,float32 w) : x(x), y(y), z(z), w(w) { }
			inline vec4(const float32 *v) : x(v[0]), y(v[1]), z(v[2]), w(v[3]) { }
			inline vec4(const vec3 &v) : x(v.x), y(v.y), z(v.z), w(1) { }
			inline vec4(const vec3 &v,float32 w) : x(v.x), y(v.y), z(v.z), w(w) { }
			inline vec4(const vec4 &v) : x(v.x), y(v.y), z(v.z), w(v.w) { }
			
			inline int32 operator==(const vec4 &v) { return (fabs(x - v.x) < EPSILON && fabs(y - v.y) < EPSILON && fabs(z - v.z) < EPSILON && fabs(w - v.w) < EPSILON); }
			inline int32 operator!=(const vec4 &v) { return !(*this == v); }
			
			inline const vec4 operator*(float32 f) const { return vec4(x * f,y * f,z * f,w * f); }
			inline const vec4 operator/(float32 f) const { return vec4(x / f,y / f,z / f,w / f); }
			...
			inline vec4 &operator*=(float32 f) { return *this = *this * f; }
			inline vec4 &operator/=(float32 f) { return *this = *this / f; }
			inline vec4 &operator+=(const vec4 &v) { return *this = *this + v; }
			inline vec4 &operator-=(const vec4 &v) { return *this = *this - v; }
			
			inline float32 operator*(const vec3 &v) const { return x * v.x + y * v.y + z * v.z + w; }
			inline float32 operator*(const vec4 &v) const { return x * v.x + y * v.y + z * v.z + w * v.w; }
		...
		inline float32 vec3::operator*(const vec4 &v) const {
			return x * v.x + y * v.y + z * v.z + v.w;
		}
		
		inline vec4 mulel(const vec4& a, const vec4& b){
		...
		/**
		* Structure storing simple 3x3 matrix.
		...
		*/
		struct mat3 {
			...
			mat3 &operator*=(float32 f) { return *this = *this * f; }
			...
			void rotate(float32 x,float32 y,float32 z,float32 angle) {
		...
		inline const vec3 vec3::operator*(const mat3 &mat) const {
			vec3 ret;
		...
		/**
		* Structure storing matrix with homogenous coordinates. It is very usefull for
		* transforming calculations. Also OpenGL store each own transformation information
		* in 4x4 matricies. Elements are stored in row major
		* \ingroup math
		*/
		struct mat4 {
			...
			mat4(float32 x,float32 y,float32 z) {
				translate(x,y,z);
			}
			mat4(const vec3 &axis,float32 angle) {
				rotate(axis,angle);
			}
			mat4(float32 x,float32 y,float32 z,float32 angle) {
				rotate(x,y,z,angle);
			}
			mat4(const mat3 &m) {
			...
			mat4 &operator*=(float32 f) { return *this = *this * f; }
			...
			bool operator==(const mat4 &m){
				for (int32 i=0; i < 16; i++)if (m[i] != mat[i]) return false;
				return true;
			}
			...
			void reflect(float32 x,float32 y,float32 z,float32 w) {
			...
			void look_at(const float32 *eye,const float32 *dir,const float32 *up) {
				look_at(vec3(eye),vec3(dir),vec3(up));
			}
			void frustum(float32 left, float32 right, float32 bottom, float32 top, float32 near, float32 far){
		
				float32 A = (right + left) / (right - left);
				float32 B = (top + bottom) / (top - bottom);
				float32 C = - (far + near) / (far - near);
				float32 D = (- 2.0 * far * near) / (far - near);
				float32 E = (2.0 * near) / (right - left);
				float32 F = (2.0 * near) / (top - bottom);
		
				mat[0] = E; mat[1] = 0; mat[2] = 0; mat[3] = 0;
				mat[4] = 0; mat[5] = F; mat[6] = 0; mat[7] = 0;
				mat[8] = A; mat[9] = B; mat[10]= C; mat[11]= -1;
				mat[12]= 0; mat[13]= 0; mat[14]= D;mat[15]= 0;
		
			}
			
			void copy (const mat4 &mc){
				for (int32 i=0; i < 16; i++) mat[i] = mc[i];
			}
			
			void get_row (int32 row, vec4 & rowv){
				rowv = vec4(mat[0+row],mat[4+row], mat[8+row], mat[12+row]);
			}
			
			vec4 get_rowv (int32 row){
				return vec4(mat[0+row],mat[4+row], mat[8+row], mat[12+row]);
			}
		...
		inline const vec3 vec3::operator*(const mat4 &mat) const {
			vec3 ret;
			ret[0] = mat[0] * v[0] + mat[1] * v[1] + mat[2] * v[2];
			ret[1] = mat[3] * v[0] + mat[4] * v[1] + mat[5] * v[2];
			ret[2] = mat[6] * v[0] + mat[7] * v[1] + mat[8] * v[2];
		...
		/**
		* This is a quaternion implementation.
		* \ingroup math
		*/
		struct quat {
			
			quat() : x(0), y(0), z(0), w(1) { }
			quat(const vec3 &dir,float32 angle) {
				set(dir,angle);
			}
			quat(float32 x,float32 y,float32 z,float32 angle) {
				set(x,y,z,angle);
			}
			quat(const mat3 &m) {
				float32 trace = m[0] + m[4] + m[8];
				if(trace > 0.0) {
					float32 s = sqrt(trace + 1.0f);
					q[3] = 0.5f * s;
					s = 0.5f / s;
					q[0] = (m[5] - m[7]) * s;
					q[1] = (m[6] - m[2]) * s;
					...
					int32 j = next[i];
					int32 k = next[j];
					float32 s = sqrt(m[3 * i + i] - m[3 * j + j] - m[3 * k + k] + 1.0f);
					q[i] = 0.5f * s;
					if(s != 0) s = 0.5f / s;
			...
			void set(const vec3 &dir,float32 angle) {
			...
			void set(float32 x,float32 y,float32 z,float32 angle) {
				set(vec3(x,y,z),angle);
			}
			
			void slerp(const quat &q0,const quat &q1,float32 t) {
			...
			{
				float32 v_coef = w * w - x * x - y * y - z * z;
				float32 u_coef = 2.0f * (src.v[0] * x + src.v[1] * y + src.v[2] * z);
				float32 c_coef = 2.0f * w;
